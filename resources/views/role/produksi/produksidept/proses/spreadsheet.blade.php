    ...
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-2xl p-6">
        ...
        <div>
            ...
            @if (session('error_duplikat'))
                <div id="alert-duplikat"
                    class="bg-red-50 border-l-4 border-red-500 text-red-800 p-4 rounded-lg mb-6 shadow-sm relative transition duration-150 ease-in-out"
                    role="alert">
                    <div class="flex items-start">
                        ...
                        <!-- Tombol Close (Silang) -->
                        <button type="button"
                            class="ml-auto flex-shrink-0 text-red-400 hover:text-red-600 focus:outline-none transition duration-150"
                            onclick="document.getElementById('alert-duplikat').remove()">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function initRow(row) {
                ...
                function calculate() {
                    ...
                    // SESUDAH — baris Input (DRIK) selalu ditampilkan untuk semua proses
                    const inputDrikRow = inputDrik?.closest('.input-drik-row');
                    if (inputDrikRow) inputDrikRow.style.display = '';
                    ...
                    if (proc === 'SORTPACKING') {
                        ...
                        // RUMUS BARU: Input (DRIK) = Karantina + OK + JT Drik (otomatis, readonly)
                        const inputCalc = karantinaVal + okVal + jtdrikVal;
                        if (inputDrik) {
                            inputDrik.value = formatNumber(Math.floor(inputCalc));
                            inputDrik.readOnly = true;
                        }

                        // outputdrik tetap = karantina + ok (tidak termasuk JT, karena JT sudah disisihkan)
